...
{
    public class CloneInjector : LoopInjection
    {
        ...
        private static object GetClone(PropertyInfo sp, object val)
        {
            ...
            if (sp.PropertyType.IsArray)
            {
                ...
                for (var index = 0; index < arr.Length; index++)
                {
                    ...
                    arrClone.SetValue(Activator.CreateInstance(a.GetType()).InjectFrom<CloneInjector>(a), index);
                }
                ...
                if (sp.PropertyType.GetGenericTypeDefinition().GetInterfaces().Contains(typeof(IEnumerable)))
                {
                    ...
                    foreach (var o in val as IEnumerable)
                    {
                        var listItem = genericType.IsValueType || genericType == typeof(string)
                            ? o
                            : Activator.CreateInstance(genericType).InjectFrom<CloneInjector>(o);
